...
What: <qué se hizo concretamente>
...
El cuerpo (`What:` / `For:` / `Impact:`) puede estar en inglés o español.
...
What: Creates EmailVerificationToken on register, sends link via SMTP,
adds POST /api/v1/auth/verify-email endpoint
...
What: Catches jwt.ErrTokenExpired in the refresh handler and returns 401
For: Expired tokens were causing unhandled panics in the refresh endpoint
Impact: Users now get a clear 401 and are redirected to login

# Actualización de dependencias con CVE
chore(deps): bump golang.org/x/crypto v0.36.0 → v0.37.0 — no known CVEs

What: Updated x/crypto to latest patch in go.mod and go.sum
For: Keep dependencies current; ran govulncheck ./... — no vulnerabilities found
Impact: No functional change; bcrypt behavior unchanged

# Documentación
docs(api): add endpoint reference for POST /api/v1/auth/verify-email

What: Added full request/response example to api-endpoints.md
For: Endpoint was missing from the API docs making it hard to discover
Impact: Developers can now find and test the email verification endpoint

# Migración de BD
chore(db): add locale column to users table

What: New migration 000004_add_locale_to_users.up.sql adds locale VARCHAR(10)
For: Store user language preference to persist it across sessions
Impact: Requires running: go run ./cmd/migrate/main.go up
